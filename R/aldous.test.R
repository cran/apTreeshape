...
function(tree, xmin=20, ...) {
	...
	plot(clade, pch=20, xlab=xlab, log="xy", ylab=ylab, xlim=c(xmin, max(clade[1,])+10), ...)

# 	x=max(clade[1,])-10

	if (xmin>=10) {
		x=xmin+5
		text(x=x, y=3/2+.1, label="PDA model", col=4)
	...
	lines(x, coef[2]*x+coef[1], col=2)
	
# 	abline(log(3/2)/log(10), 0)
# 	abline(-log(4)/log(10), 1)
